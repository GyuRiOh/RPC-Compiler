...
void server_baby::SystemLogger::Console(const wchar_t* type, const int logLevel, const wchar_t* stringFormat, ...)
{
	...
	wprintf_s(L"[Type : %ws]", type);
	wprintf_s(L"[Log Level : %d] ", logLevel);
	wprintf_s(L"[Log : %ws]\n", message);
}
...
void server_baby::SystemLogger::SetTimeStamp(wchar_t* pathBuffer)
{
	...
	wsprintf((LPWSTR)pathBuffer, L"[%d/%02d/%02d/%d/%d/%d]",
		t.tm_year + 1900,
		t.tm_mon + 1,
		t.tm_mday,
		t.tm_hour,
...

{
	...
	wsprintf(string, L"%d%02d%02d_%02d_",
		t.tm_year + 1900,
		t.tm_mon + 1,
		t.tm_mday,
		t.tm_hour);
