#ifndef __LAN__RPC__MAKER__
#define __LAN__RPC__MAKER__
...
namespace server_baby
{
	...
	public:
		explicit LanRPCMaker(const char* SC_Protocol = "Lan_RPC_SC_Protocol.ini", const char* CS_Protocol = "Lan_RPC_CS_Protocol.ini") : procCount_SC_(NULL), procCount_CS_(NULL)
		{
			//파서 싱글톤 좀 오버엔지니어링
			//SC Protocol 파싱
			...
			{
				SystemLogger::GetInstance()->LogText(L"LanRPCMaker", LEVEL_ERROR, L"SC_CS Protocol Name - Not Same");
				CrashDump::Crash();
			}
	...
	private:
		void CreateRequestCode()
		{
			...
			if (fopen_s(&stream, fileName, "w") == 0)
			{
				...
				fprintf(stream, "#include \"NetRoot/LanServer/LanPacket.h\"\n");
				fprintf(stream, "#include \"NetRoot/LanServer/LanSessionID.h\"\n");
				...
				for (int i = 0; i < procCount_SC_; i++)
				{
					...
					fprintf(stream, "LanSessionID sessionID)\n");
					fprintf(stream, "\t\t{\n");
		...
		void CreateResponseCode()
		{
			...
			if (fopen_s(&stream, fileName, "w+") == 0)
			{
				...
				fprintf(stream, "#include \"NetRoot/LanServer/LanSessionID.h\"\n");
				fprintf(stream, "#include \"NetRoot/LanServer/LanPacketSet.h\"\n");
				...
				//생성자
				fprintf(stream, "\tclass %s_CS_Stub final\n", serverName_);
				fprintf(stream, "\t{\n");
				fprintf(stream, "\tpublic:\n");
				...
				//msgPack Proc
				fprintf(stream, "\t\tbool PacketProc(LanPacketSet* msgPack)\n");
				fprintf(stream, "\t\t{\n");
				fprintf(stream, "\t\t\tswitch (msgPack->GetType())\n");
				fprintf(stream, "\t\t\t{\n");
				fprintf(stream, "\t\t\tcase eLAN_RECVED_PACKET_SET:\n");
				fprintf(stream, "\t\t\t{\n");
				fprintf(stream, "\t\t\t\twhile (msgPack->GetSize() > 0)\n");
				fprintf(stream, "\t\t\t\t{\n");
				fprintf(stream, "\t\t\t\t\tLanDummyPacket* packet = nullptr;\n");
				...
				//나눠주기
				fprintf(stream, "\t\tbool PacketProc(LanSessionID sessionID, LanDummyPacket* msg)\n");
				fprintf(stream, "\t\t{\n");
